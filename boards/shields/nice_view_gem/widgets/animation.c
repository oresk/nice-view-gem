...
void draw_animation(lv_obj_t *canvas) {
    ...
    lv_obj_t *art = lv_animimage_create(canvas);
    lv_obj_center(art);

    lv_animimage_set_src(art, (const void **)anim_imgs, 16);
    lv_animimage_set_duration(art, CONFIG_NICE_VIEW_GEM_ANIMATION_MS);
    lv_animimage_set_repeat_count(art, LV_ANIM_REPEAT_INFINITE);
    lv_animimage_start(art);
#else
    ...
    lv_image_set_src(art, anim_imgs[random_index]);
#endif
